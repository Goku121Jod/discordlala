...
async def handle_channel_messages(message):
    ...
    try:
        ...
        embed = discord.Embed(title="Crypto MM", description="Welcome to our automated cryptocurrency Middleman system!", color=EMBED_COLOR_GREEN)
        embed.set_footer(text="Created by: Exploit")
        await channel.send(embed=embed)

        # Warning embed
        embed = discord.Embed(title="Please Read!", description="Ensure all deal conversations happen here.", color=EMBED_COLOR_RED)
        await channel.send(embed=embed)
    ...
    if message.content.lower() in ["sending", "receiving"]:
        ...
        channel_data[channel.id]["sender_confirmed" if message.author.id in [u for u, r in channel_data[channel.id]["roles"].items() if r == "Sender"] else "receiver_confirmed"] = True
...
async def handle_release_command(message):
    ...
    try:
        ...
        embed = discord.Embed(title="Deal Completed", description="Thank you for using the auto middleman service.", color=EMBED_COLOR_GREEN)
        await message.channel.send(embed=embed)
